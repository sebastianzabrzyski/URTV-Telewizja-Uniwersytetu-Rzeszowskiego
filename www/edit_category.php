<!-- Strona z edycją kategorii materiałów wideo -->
  ...
  FROM categories WHERE ID = {$id_kategorii};";
    ...
    if ($wynik->num_rows > 0) {
        while($wiersz = $wynik->fetch_assoc()) {
            $id_grupy_zmienianej = $wiersz["Group_ID"];
            if($id_grupy_zmienianej == null) $id_grupy_zmienianej = "0";
            $nazwa_kategorii = $wiersz["Name"];
            break;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'edit_category') {
    ...
    $zapytanie_SQL = "UPDATE categories SET
  Name = '{$kategoria}',
  Group_ID = {$id_grupy}
