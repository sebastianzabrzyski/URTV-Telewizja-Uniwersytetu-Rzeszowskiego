...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $pokazac = false;
    $plik_istnieje = false;
    $active_key = $row["Streamkey_active"];
    $last_key = $row["Streamkey_last"];
    ...
    if($active_key == $last_key) {
      ...
      if($plik_istnieje === true) {
        ...
        $query = "UPDATE streams SET Streamkey_last = NULL, Streamkey_active = {$new_key} WHERE Streamkey_active = '{$active_key}';";
      ...
      if($plik_istnieje === true) {
        $pokazac = true;
        $query = "UPDATE streams SET Streamkey_last = {$active_key} WHERE Streamkey_active = '{$active_key}';";
        $result2 = queryDB($conn,$query);
      }
    }
    ...
    if($pokazac === true ) {
      ...
      $planowana_data = $row["Planned_date"];
...
$query_count = "SELECT count(ID) FROM movies WHERE Verified = 'Tak';";
$result = queryDB($conn,$query_count);
$row = $result->fetch_row();
$wszystkie_wyniki = $row[0] + $wszystkie_wyniki;
$query = "SELECT movies.ID, User_ID, Title, Views, Author, Filename, movies.Date, Describtion, movies.Password, Login FROM movies INNER JOIN users ON movies.User_ID = users.ID WHERE Verified = 'Tak' ORDER BY movies.Date DESC LIMIT {$pokazywane_wyniki} OFFSET {$pominiete_wyniki};";
$result = queryDB($conn,$query);
$id_filmu = null;
