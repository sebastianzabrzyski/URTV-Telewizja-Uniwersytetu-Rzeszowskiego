<!-- Strona z dodawaniem nowej grupy kategorii materiałów filmowych -->
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'add_group') {
    ...
    $nazwa_grupy = $polaczenie_BD->real_escape_string($nazwa_grupy);

    $zapytanie_SQL_count = "SELECT count(ID) FROM categories_groups WHERE Name = '{$nazwa_grupy}';";
    $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL_count);
    $wiersz = $wynik->fetch_row();
    $taka_sama_nazwa = $wiersz[0];
    ...
    if($taka_sama_nazwa > 0) {
        ...
        $zapytanie_SQL = "INSERT INTO categories_groups (Name)
      VALUES ('{$nazwa_grupy}')";
