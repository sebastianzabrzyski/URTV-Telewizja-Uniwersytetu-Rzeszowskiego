...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'add_category') {
    ...
    $nazwa_kategorii = $polaczenie_BD->real_escape_string($nazwa_kategorii);
    $id_grupy = $polaczenie_BD->real_escape_string($id_grupy);

    $zapytanie_SQL_count = "SELECT count(ID) FROM categories WHERE Name = '{$nazwa_kategorii}';";
    ...
    if($taka_sama_nazwa > 0) {
        ...
        $zapytanie_SQL = "INSERT INTO categories (Name, Group_ID)
