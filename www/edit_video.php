...
$query = "SELECT User_ID FROM movies WHERE ID = '{$id_filmu}';";
$result = queryDB($conn,$query);
$row = $result->fetch_row();
$id_wlasciciela = $row[0];

if($id_wlasciciela == $user_id) {

  $query = "SELECT movies.User_ID, Title, Views, Author, Filename, movies.Date, Describtion, movies.Password, Login FROM movies INNER JOIN users ON users.ID = movies.User_ID WHERE movies.ID = {$id_filmu};";
  $result = queryDB($conn,$query);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'update_video') {
  ...
  if($result === TRUE) {
    ...
    $query = "DELETE FROM categories_videos WHERE Movie_ID = '{$id_filmu}';";
    $result = queryDB($conn,$query);
    ...
    if ($tagi_nowe == "") {
      ...
      $query = "DELETE FROM tags WHERE Movie_ID = '{$id_filmu}';";
      ...
      foreach ($tagi_tablica_nowa as $tag_nowy) {
        ...
        if (!in_array($tag_nowy, $tagi_tablica)) {
          $tag_nowy = $conn->real_escape_string($tag_nowy);
      ...
      foreach ($tagi_tablica as $tag_stary) {
        ...
        if (!in_array($tag_stary, $tagi_tablica_nowa)) {
          ...
          $query = "DELETE FROM tags WHERE Name = '{$tag_stary}';";
          $result = queryDB($conn,$query);
        }
      }
    }
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    array_push($kategorie_filmu_ids,$row["Category_ID"]);
