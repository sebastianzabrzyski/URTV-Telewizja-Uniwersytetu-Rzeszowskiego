...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'register_form')
{
  ...
  sprawdzPoprawnosc($password_1,"a-zA-Z0-9ąćęłńóśźżĄĆĘŁŃÓŚŹŻ_ \W",8,0) == "Poprawne" &&
  ...
  if($dane_poprawne == 3) {
    ...
    $query = "INSERT INTO unverified_users (Login, Password, Name, Surname, Email, Code)
    VALUES (lower('{$login}'), '{$password}', '{$imie}','{$nazwisko}', '{$email}', '{$kod_weryfikacyjny}')";
