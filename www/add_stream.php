...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'update_stream')
{
  ...
  $data_planowania = $conn->real_escape_string($data_planowania);
  $autor = $conn->real_escape_string($autor);

  $query = "INSERT INTO streams (User_ID, Title, Views, Date, Describtion, Password, Streamkey_last, Streamkey_active, Filename, Planned_date, Author)
  VALUES ('{$user_id}', '{$tytul}',0, '{$data}','{$opis}',{$haslo}, NULL, '{$new_key}', '{$nazwa_pliku}', '{$data_planowania}', '{$autor}') ON DUPLICATE KEY UPDATE
  ...
  if($result === TRUE) {
    ...
    try {
      $id_streamu = $conn->insert_id;
    } catch (Exception $e) {

    }

    $query = "DELETE FROM categories_videos WHERE Stream_ID = '{$id_streamu}';";
    $result = queryDB($conn,$query);

    foreach($kategorie as $kategoria) {
      $kategoria = $conn->real_escape_string($kategoria);
      $query = "INSERT INTO categories_videos (Movie_ID, Stream_ID, Category_ID)
    ...
    $query = "DELETE comments_streams, likes_streams FROM comments_streams LEFT JOIN likes_streams ON comments_streams.Stream_ID = likes_streams.Stream_ID WHERE comments_streams.Stream_ID = '{$id_streamu}';";
    $result = queryDB($conn,$query);
...
$query = "SELECT Name, Surname FROM users WHERE ID = {$user_id};";
$result = queryDB($conn,$query);
...
try {
  ...
  $query = "SELECT ID FROM streams WHERE User_ID = {$user_id};";
  $result = queryDB($conn,$query);
  ...
  if ($result->num_rows > 0) {
    ...
    $query = "SELECT streams.User_ID, Title, streams.Date, Planned_date, Author, Describtion, Streamkey_active, Streamkey_last, streams.Password FROM streams INNER JOIN users ON users.ID = streams.User_ID WHERE streams.ID = {$id_streamu};";
    $result = queryDB($conn,$query);
    ...
    if ($result->num_rows > 0) {
      ...
      while($row = $result->fetch_assoc()) {
        ...
        $autor = $row["Author"];
        $streamkey_db = $row["Streamkey_active"];
        $streamkey_last_db = $row["Streamkey_last"];
        $data_planowania = $row["Planned_date"];
        ...
        if($streamkey_db == $streamkey_last_db) {
          ...
          if (!file_exists("{$streamy_sciezka}/{$streamkey_db}.m3u8")) {
            ...
            $new_key = mt_rand(1000000000, 9999999999);
            $query = "UPDATE streams SET Streamkey_active = {$new_key}, Streamkey_last = NULL WHERE Streamkey_active = {$streamkey_db};";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    array_push($kategorie_streamu_ids,$row["Category_ID"]);
