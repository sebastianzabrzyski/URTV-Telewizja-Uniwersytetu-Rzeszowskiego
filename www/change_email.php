...
if(isset($_GET['code'])) {
  ...
  $query = "SELECT ID, User_ID, Code, Time, Email
  FROM change_email
  WHERE Code = '{$kod}';";
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      $czas_wyslania = $row['Time'];
      $id_resetu = $row['ID'];
      $id_uzytkownika = $row['User_ID'];
      $kod_db = $row['Code'];
      $nowy_email = $row['Email'];
      $aktualny_czas = time();
      $roznica = $aktualny_czas - $czas_wyslania;

      if($roznica > 86400) {
        $query = "DELETE FROM change_email WHERE ID = '{$id_resetu}';";
        $result = queryDB($conn,$query);
        pokazKomunikat("Upłynął czas na zmianę adresu e-mail");
      } else {
        $query = "UPDATE users SET Email = '{$nowy_email}' WHERE ID = {$id_uzytkownika};";
        $result = queryDB($conn,$query);
