...
$id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
$zapytanie_SQL = "SELECT Space, Name, Surname FROM users WHERE ID = {$id_uzytkownika};";
$wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
       ...
       $format != "mpg" &&  $format != "rmvb" && $format != "wmv" ) {
        ...
        if ($rozmiar_mb > $pozostalo_miejsca) {
            ...
            if ($uploadOk == 0) {
                ...
                $media_info = exec("/var/www/html/ffmpeg/ffprobe -show_streams {$source_file}",$output,$wynik);
                ...
                if($format != "mp4" || $dobry_kodek == 0) {
                    ...
                    try {
                        ...
                        //$key2 = 's';
                        //$value2 = '872x490';
                        $FFmpeg = new FFmpeg($ffmpeg_sciezka);
                ...
                if ($przekonwertowane == 1 || ($uploadOk == 1 && move_uploaded_file($source_file, $target_file))) {
                    ...
                    $zapytanie_SQL = "INSERT INTO movies (User_ID, Title, Views, Size, Date, Describtion, Password, Verified, Filename, Author)
          VALUES ('{$id_uzytkownika}', '{$tytul}',0, '{$rozmiar_mb}', '{$data}','{$opis}',{$haslo}, '{$verified}', '{$nazwa_pliku}','{$autor}')";

                    $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
                    if($wynik === TRUE) {
                        $id_filmu = $polaczenie_BD->insert_id;
                        if($uprawnienia == "Użytkownik") {
              ...
              VALUES (NULL, {$id_filmu}, '{$kategoria}')";
                        ...
                        if ($tagi != "") {
                            ...
                            foreach ($tagi_tablica as $tag) {
                                $tag = trim($tag);
                                $tag = $polaczenie_BD->real_escape_string($tag);
                ...
                VALUES ('{$tag}', {$id_filmu})";
                        ...
                        try {
                            ...
                            // subskrybenci
                            $id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
                            $zapytanie_SQL = "SELECT DISTINCT User_ID FROM subscription WHERE (Author_ID = {$id_uzytkownika} OR Author_ID = 0) AND NOT Author_ID = -1;";
                            $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
                            if ($wynik->num_rows > 0) {
                                while($wiersz = $wynik->fetch_assoc()) {
                                    $id_subskrybenta = $wiersz["User_ID"];

                                    $zapytanie_SQL2 = "SELECT DISTINCT User_ID FROM subscription WHERE User_ID = {$id_subskrybenta} AND Author_ID = -1;";
                                    ...
                                    $zapytanie_SQL2 = "SELECT Email FROM users WHERE ID = {$id_subskrybenta};";
                                    $wynik2 = wykonajSQL($polaczenie_BD,$zapytanie_SQL2);
                                    if ($wynik2->num_rows > 0) {
                                    ...
                                    $wiadomosc = "Witaj,\n\nw serwisie TV UR pojawił się nowy film pod tytułem \"{$tytul}\", przesłany przez użytkownika {$nazwa_uzytkownika}.\n\nMożesz go obejrzeć na stronie: {$adres}\n\nJeżeli nie chcesz otrzymywać kolejnych powiadomień, {$rezygnacja}\n\nPozdrawiamy!";
                                    wyslijEmail($email,$temat,$wiadomosc,"noreply@urtv.tk","TV UR");
                                }
                            }

                            // newsletter
                            $zapytanie_SQL = "SELECT DISTINCT Email FROM newsletter;";
                            $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
                            if ($wynik->num_rows > 0) {
                                while($wiersz = $wynik->fetch_assoc()) {
