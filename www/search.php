...
$query_count = "SELECT count(ID) FROM movies {$where};";

$result = queryDB($conn,$query_count);
$row = $result->fetch_row();
$wszystkie_wyniki = $row[0];
$sortowanie = $conn->real_escape_string($sortowanie);
$query = "SELECT movies.ID, User_ID, Title, Filename, Author, Views, movies.Date, Describtion, movies.Password, Login, COALESCE(polubienia.Suma, 0) AS Likes
          FROM movies
