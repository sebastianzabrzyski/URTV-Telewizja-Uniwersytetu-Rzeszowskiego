...
if(isset($_GET['id'])) {
  ...
  $query = "SELECT Login, Privileges, Space FROM users WHERE ID = {$id_uzytkownika};";
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      $login = $row["Login"];
      $uprawnienia_stare = $row["Privileges"];
      $miejsce_stare = $row["Space"];
      break;
    }
  } else {
    przekierowanie("./index.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'update_user') {
  ...
  $nowe_uprawnienia = $conn->real_escape_string($nowe_uprawnienia);
  $nowe_miejsce = $conn->real_escape_string($nowe_miejsce);
  $id_uzytkownika = $conn->real_escape_string($id_uzytkownika);
