...
$zapytanie_SQL = "SELECT Name, Surname, Email FROM users WHERE ID = {$id_uzytkownika};";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'newsletter_form')
{
    ...
    if($wybor == "no_any_ns") {
        ...
        $zapytanie_SQL = "DELETE FROM subscription WHERE User_ID = {$id_uzytkownika} AND Author_ID = 0;";
        $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        $zapytanie_SQL = "INSERT INTO subscription (User_ID, Author_ID) VALUES ({$id_uzytkownika}, -1)";
        $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        pokazKomunikat("Ustawienia subskrybcji zostały zapisane");

    } elseif ($wybor == "all_ns") {

        //	$zapytanie_SQL = "DELETE FROM newsletter WHERE User_ID = {$id_uzytkownika};";
        //  $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        $id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
        $zapytanie_SQL = "DELETE FROM subscription WHERE User_ID = {$id_uzytkownika} AND Author_ID = -1;";
        $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        $zapytanie_SQL = "INSERT INTO subscription (User_ID, Author_ID) VALUES ({$id_uzytkownika}, 0)";
        $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        pokazKomunikat("Ustawienia subskrybcji zostały zapisane");

    } else {
        $id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
        $zapytanie_SQL = "DELETE FROM subscription WHERE User_ID = {$id_uzytkownika} AND (Author_ID = 0 OR Author_ID =-1);";
        $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        //$zapytanie_SQL = "SELECT ID FROM subscription WHERE User_ID = {$id_uzytkownika};";
        //  $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);

        //if ($wynik->num_rows == 0) {
        //    $id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
        //    $zapytanie_SQL = "INSERT INTO subscription (User_ID) VALUES ({$id_uzytkownika})";
        //    $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
        //  }
...
$id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
$zapytanie_SQL = "SELECT Author_ID FROM subscription WHERE User_ID = {$id_uzytkownika};";
$wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
...
$polaczenie_BD = polaczDB();
$id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
$zapytanie_SQL_count = "SELECT count(ID) FROM movies WHERE User_ID = {$id_uzytkownika};";

$wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL_count);
$wiersz = $wynik->fetch_row();
$wszystkie_wyniki = $wiersz[0];

$zapytanie_SQL = "SELECT movies.ID, User_ID, Title, Filename, Views, movies.Date, Describtion FROM movies INNER JOIN users ON movies.User_ID = users.ID WHERE User_ID = {$id_uzytkownika} ORDER BY movies.Date DESC LIMIT {$pokazywane_wyniki} OFFSET {$pominiete_wyniki};";
$wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
...
if ($wynik->num_rows > 0) {
    ...
    while($wiersz = $wynik->fetch_assoc()) {
        ...
        $zapytanie_SQL_komentarze = "SELECT COUNT(ID) FROM comments WHERE Movie_ID = {$id_filmu} ;";
        $wynik_kom = wykonajSQL($polaczenie_BD,$zapytanie_SQL_komentarze);
        $wiersz_kom = $wynik_kom->fetch_row();
        $komentarze = $wiersz_kom[0];
...
$do_wykorzystania = 0;
$id_uzytkownika = $polaczenie_BD->real_escape_string($id_uzytkownika);
$zapytanie_SQL = "SELECT SUM(Size) AS Wykorzystane FROM movies WHERE User_ID = {$id_uzytkownika};";
$wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
...
if ($wynik->num_rows > 0) {
    ...
    while($wiersz = $wynik->fetch_assoc()) {
        ...
        $wykorzystane = $wiersz["Wykorzystane"];
...
$zapytanie_SQL = "SELECT Space FROM users WHERE ID = {$id_uzytkownika};";
$wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
    ...
    <body>
        ...
        <div id="blok_glowny">
            <div class="kontener">
                ...
                <div class="naglowek_formularza"><h1>Informacje o koncie <span onclick="potwierdzUsuniecieUzytkownika(<?php echo $id_uzytkownika;?>);return false;"><a href="#">&nbsp;[Usuń konto]</a></span><span style="<?php echo $link_panel_admina; ?>"><a href="./admin.php">[Panel administratora]</a></span></h1></div>
                <div class="tlo blok_odstep_dolny">
                    <div class="blok_formularza formularz_moje_dane blok_padding_gorny blok_padding_dolny">
                        <form>
                            <div class="pole">
                                <input type="text" class="readonly centrowanie" name="login" id="login" readonly value="<?php echo $nazwa_uzytkownika; ?>">
                                <p>Login:</p>
                            </div>
                            <div class="pole">
