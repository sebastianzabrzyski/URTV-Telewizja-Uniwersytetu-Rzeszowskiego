...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'reset_password')
{
  ...
  if(isset($_POST['code'])) {
    ...
    $query = "SELECT ID, User_ID, Code, Time
    FROM reset_password
    WHERE Code = '{$kod}';";
    ...
    if ($result->num_rows > 0) {
      ...
      while($row = $result->fetch_assoc()) {
        ...
        if($roznica > 86400) {
          $query = "DELETE FROM reset_password WHERE ID = '{$id_resetu}';";
          $result = queryDB($conn,$query);
          ...
          $query = "UPDATE users SET Password = '{$nowe_haslo_bcrypt}' WHERE ID = {$id_uzytkownika};";
          $result = queryDB($conn,$query);
