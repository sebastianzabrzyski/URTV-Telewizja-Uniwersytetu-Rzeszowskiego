...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'login_form')
{
    ...
    $zapytanie_SQL = "SELECT ID, Privileges, Password
  FROM users
  WHERE (Login = '{$login}');";
    ...
    if ($wynik->num_rows > 0) {
        ...
        while($wiersz = $wynik->fetch_assoc()) {
            if ( password_verify ($haslo, $wiersz["Password"]) == true) {
                ...
                $_SESSION['privileges'] = $wiersz["Privileges"];
    ...
    $polaczenie_BD = polaczDB();
    $login = $polaczenie_BD->real_escape_string($login);
    $zapytanie_SQL = "SELECT Email, ID, Login
  FROM users
  WHERE Login = '{$login}';";
    $wynik = wykonajSQL($polaczenie_BD,$zapytanie_SQL);
    ...
    if ($wynik->num_rows > 0) {
        ...
        while($wiersz = $wynik->fetch_assoc()) {
            ...
            if($login_db == $login) {
                ...
                $zapytanie_SQL = "INSERT INTO reset_password (User_ID, Code, Time)
        VALUES ({$id_uzytkownika}, '{$kod_weryfikacyjny}', '{$aktualny_czas}')";
